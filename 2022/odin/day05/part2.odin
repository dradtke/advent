...
move :: proc(stacks: ^map[int]queue.Queue(rune), instruction: MoveInstruction) {
  grabbed := queue.Queue(rune){}
  defer queue.destroy(&grabbed)

  for i := 0; i < instruction.Count; i += 1 {
    box := queue.pop_back(&stacks[instruction.From])
    if !queue.push_back(&grabbed, box) {
      panic("cannot push box onto grabbed!")
    }
  }

  for queue.len(grabbed) > 0 {
    box := queue.pop_back(&grabbed)
    if !queue.push_back(&stacks[instruction.To], box) {
